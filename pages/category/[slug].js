...
export default function Category({category,items}){return <Layout><section className="pageHead"><h1>{category.name}</h1><p>{category.subtitle}</p></section><section className="grid">{items.map(p=><ProductCard key={p.slug} p={p}/>)}</section></Layout>}
